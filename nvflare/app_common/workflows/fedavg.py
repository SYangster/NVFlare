...
class FedAvg(WFController):
    ...
    def run(self) -> None:
        ...
        for self.current_round in range(self.num_rounds):
            self.logger.info(f"Round {self.current_round} started.")

            clients = self.sample_clients(self.min_clients)
            
            self.model.current_round = self.current_round
            results = self.broadcast_and_wait(task_name=self.task_name, min_responses=len(clients), data=self.model, targets=clients, callback=None)
            ...
            self.save_model(self.model, "/tmp/nvflare/fedavg/mode.pth")

        self.logger.info("Finished FedAvg.")
    ...
    def _check_results(results: List[FLModel]):
        ...
        for task in results.values():
            for res in task:
                if 'result' in res:
                    for _result in res['result'].values():
                        if not _result.value.params:
    ...
    def _aggregate_fn(results: List[FLModel]) -> FLModel:
        ...
        for site, _result in task_result.items():
        ...
        aggr_result = FLModel(
            params=aggregated_dict,
            params_type=ParamsType.FULL,#,task_result.values()[0].params_type,
            meta={"nr_aggregated": len(task_result), "current_round": 0}#,task_result.values()[0].meta["current_round"]},
        )
    ...
    def aggregate(self, results: List[FLModel], aggregate_fn=None) -> FLModel:
        ...
        self.logger.info(f"aggregating {len(results)} update(s) at round {self.current_round}")
        try:
