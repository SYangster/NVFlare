...
class ClientSideExecutor(Executor):#, TaskController):
    ...
    def start_run(self, fl_ctx: FLContext):
        ...
        self.controller = self.engine.get_component(self.controller_id)
        ...
        self.persistor = self.engine.get_component(self.persistor_id) #do we want a special persistor for ccwf, or use the persistor only in cyclic?
    ...
    def handle_event(self, event_type: str, fl_ctx: FLContext):
        if event_type == EventType.START_RUN:
            ...
            #print(f"\n\t BEFORE_PULL_TASK in client_ctl_executor \n")
            #self.fire_event(EventType.BEFORE_PROCESS_TASK_REQUEST, fl_ctx) #testing
    ...
    def finalize(self, fl_ctx: FLContext):
        ...
        with self.finalize_lock:
            ...
            #self.controller.communicator.finalize_run(fl_ctx)
    ...
    def _try_broadcast_final_result(
        ...
        result = self.controller.persistor.load(fl_ctx)
        ...
        resp = self.controller.broadcast_and_wait(#self.broadcast_and_wait(
            task=task,
            targets=targets,
