...
class ColorFormatter(BaseFormatter):
    def __init__(
        ...
        Args:
            ...
            datefmt (str): date/time format string. Defaults to '%Y-%m-%d %H:%M:%S'.
            style (str): style character '%' '{' or '$' for format string.
            level_colors (Dict[str, int]): dict of levelname: colors.
            logger_names (List[str]): list of logger names to apply logger color.
    ...
    def format(self, record):
        ...
        if record.levelno <= logging.INFO and any(
            ...
            log_fmt = ansi_sgr(self.level_colors.get(record.levelname, "")) + self.fmt + ansi_sgr(ANSIColor.RESET)
...
class JsonFormatter(BaseFormatter):
    def __init__(
        ...
        Args:
            fmt (str): format string which uses LogRecord attributes. Attributes used for JSON keys.
            datefmt (str): date/time format string. Defaults to '%Y-%m-%d %H:%M:%S'.
            style (str): style character '%' '{' or '$' for format string.
...
def read_log_config(file, dir_path: str = "") -> dict:
    """
    ...
    try:
        with open(file, "r") as f:
            ...
            config = json.load(f)
        if dir_path:
            config = update_dict_filenames(config, dir_path)
        return config
    except Exception as e:
        raise ValueError(f"Unrecognized logging configuration format. Failed to parse JSON: {e}.")


def configure_logging(workspace: Workspace, dir_path: str = ""):
...
def add_log_file_handler(log_file_name):
    ...
    main_file_handler = root_logger.handlers[1]
    file_handler = RotatingFileHandler(log_file_name, maxBytes=20 * 1024 * 1024, backupCount=10)
    file_handler.setLevel(main_file_handler.level)
    file_handler.setFormatter(main_file_handler.formatter)
