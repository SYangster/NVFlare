...
class ServerEngine(ServerEngineInternalSpec, StreamableEngine):
    ...
    def configure_logging(self, job_id) -> dict:
        result = None
        try:
            result = self.send_command_to_child_runner_process(
                job_id=job_id,
                command_name=ServerCommandNames.CONFIGURE_LOGGING,
                command_data={},
            )
        except Exception as ex:
            self.logger.error(f"Failed to configure_logging for JOB: {job_id}: {secure_format_exception(ex)}")
