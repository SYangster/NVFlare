...
class JobCommandModule(CommandModule, CommandUtil, BinaryTransfer):
    ...
    def get_spec(self):
        return CommandModuleSpec(
            ...
            cmd_specs=[
                ...
                CommandSpec(
                    name=AdminCommandNames.CONFIGURE_LOG,
                    description="configure logging of a job",
                    usage=f"{AdminCommandNames.CONFIGURE_LOG} job_id server|client|all|[client_name] [level]|[file]",
                    handler_func=self.configure_log,
    ...
    def configure_log(self, conn: Connection, args: List[str]):
        job_id = args[1]
        engine = conn.app_ctx
        if not isinstance(engine, ServerEngine):
            raise TypeError("engine must be ServerEngine but got {}".format(type(engine)))
        ...
        message = new_message(conn, topic=TrainingTopic.CONFIGURE_LOG, body=config, require_authz=False) # TODO change require_authz
        message.set_header(RequestHeader.JOB_ID, str(job_id))

        if target_type == self.TARGET_TYPE_SERVER:
            #engine.

            engine.configure_log(str(job_id), config)
            # if not self._start_app_on_server(conn, job_id):
            #     return
            #conn.append_error("start_app command only supports client app start.")
            #return
        ...
        # message = new_message(conn, topic=TrainingTopic.CONFIGURE_LOG, body="", require_authz=False)
        # message.set_header(RequestHeader.JOB_ID, str(job_id))
        # clients = engine.get_clients()
        # if clients:
        #     conn.set_prop(self.TARGET_CLIENT_TOKENS, [x.token for x in clients])
